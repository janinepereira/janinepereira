# Olá, sou a Janine Almeida – Analista de QA (Quality Assurance) 👋

👩‍💻 **Sobre Mim**
---
Profissional de Quality Assurance focada em garantir a excelência e a estabilidade de softwares através da automação de testes. Casada, mãe de pet e apaixonada por viagens. 

Atualmente desenvolvo projetos de automação robustos e dinâmicos, com foco em testes End-to-End (E2E), testes de API e performance, buscando sempre escalar a cobertura de testes e otimizar processos.

Tenho interesse especial em:
* Automação de Testes E2E com **Cypress**
* Geração de massa de dados e testes orientados a dados (*Data-Driven Testing*)
* Behavior-Driven Development (**BDD**) com **Gherkin**
* Testes de API com **Rest Assured**
* Testes de Performance com **JMeter**

Minha visão é voltada para a prevenção de falhas desde o início do desenvolvimento, garantindo entregas de alto valor e melhor experiência para o usuário final.

🧠 **Hard Skills**
---

### 💻 Automação E2E & Web
<p align="left">
  <img src="https://img.shields.io/badge/-cypress-%23E5E5E5?style=for-the-badge&logo=cypress&logoColor=058a5e" alt="Cypress" />
  <img src="https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E" alt="JavaScript" />
</p>

### ⚙️ API & Performance
<p align="left">
  <img src="https://img.shields.io/badge/-Rest%20Assured-43B02A?style=for-the-badge&logo=java&logoColor=white" alt="Rest Assured" />
  <img src="https://img.shields.io/badge/-JMeter-%23D22128?style=for-the-badge&logo=apachejmeter&logoColor=white" alt="JMeter" />
</p>

### 🛠️ Ferramentas & Versionamento
<p align="left">
  <img src="https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white" alt="Git" />
  <img src="https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  <img src="https://img.shields.io/badge/Visual%20Studio%20Code-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white" alt="VS Code" />
</p>

📁 **Projetos**
---
Os repositórios deste perfil representam aprendizado contínuo e aplicação prática, com foco em:
* Desenvolvimento de scripts de automação complexos
* Manipulação dinâmica de massa de dados (ex: via planilhas Excel e Faker.js)
* Integração Contínua (CI) com GitHub Actions
* Boas práticas e código de teste sustentável

📌 Confira os repositórios fixados abaixo para ver o código em ação.

🧭 **Como eu trabalho**
---
* Código de teste limpo, legível e de fácil manutenção
* Qualidade como cultura da equipe, não apenas uma etapa final
* Decisões técnicas alinhadas com as regras de negócio
* Testes baseados em dados reais para simular o comportamento do usuário

🔍 **O que você vai encontrar aqui**
---
* Projetos de automação reais e estruturados
* Históricos de commits compreensíveis
* Organização pensada para leitura técnica e avaliação

🤝 **Vamos conversar**
---
Estou aberta a trocar ideias sobre Qualidade de Software, automação, ferramentas de teste e viagens!

<a href="https://www.linkedin.com/in/jpereiraalmeida" target="_blank">
  <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>
